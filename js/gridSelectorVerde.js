...
;( function( window ) {
	...
	function gridSelectorVerde( el, options ) {	
		this.el = el;
		this.options = extend( this.defaults, options );
		this._init();
	}

	gridSelectorVerde.prototype = {
		defaults : {
			rows : 5,
			columns : 5,
			maxcolumns : 5,
		...
		_init : function() {
			this.trigger =  this.el.querySelector( 'span.gt-gridVerde-icon' );
			this.gridItems = Array.prototype.slice.call( this.el.querySelectorAll( 'div.gt-gridVerde-select > div' ) );
			...
			this.gridItems.forEach( function( el, i ) {
				classie.add( el, 'gt-gridVerde-selected' );
			} );
			this._initEvents();
		},
		_initEvents : function() {
			...
			if( Modernizr.touch ) {
				this.trigger.addEventListener( 'click', function( ev ) { classie.add( self.el, 'gt-gridVerde-control-open' ); } );
			}
			...
			this.gridItems.forEach( function( el, i ) {
				...
				el.addEventListener( 'click', function(ev) {
					...
					if( Modernizr.touch ) {
						classie.remove( self.el, 'gt-grid-controlVerde-open' );
					}
				} );
			} );
		...
		_highlightGridItems : function( pos ) {
			...
			this.gridItems.forEach( function( el, i ) {
				...
				if( elRow <= itemRow && elColumn <= itemColumn) {
					classie.add( el, 'gt-gridVerde-hover' );
				}
				else {
					classie.remove( el, 'gt-gridVerde-hover' );
				}
			} );
		...
		_selectGridItems : function( pos ) {
			var self = this;
			this.gridItems.forEach( function( el, i ) {
				if( classie.has( el, 'gt-gridVerde-hover' ) ) {
					classie.add( el, 'gt-gridVerde-selected' );
				}
				else {
					classie.remove( el, 'gt-gridVerde-selected' );
				}
			} );
	...
	window.gridSelectorVerde = gridSelectorVerde;
